...
from MNSIM.Latency_Model.Model_latency import Model_latency,Model_latency_ou
...
class Model_energy_ou(Model_energy):
    def __init__(self, NetStruct, SimConfig_path, model_power=None, model_latency=None, multiple=None, TCG_mapping=None):
        super().__init__(NetStruct, SimConfig_path, model_power, model_latency, multiple, TCG_mapping)
        ...
        self.model_latency = Model_latency_ou(NetStruct,SimConfig_path,multiple,TCG_mapping)
        self.model_latency.calculate_model_latency_ou() # 重算ou模式下的时延
        self.model_power.calculate_model_power_ou() # 重算ou模式下的功率
        self.calculate_model_energy_ou() # 重新计算能耗

    def calculate_model_energy_ou(self):
        #print(self.model_latency.total_buffer_r_latency)
        self.global_buf = buffer(SimConfig_path=self.SimConfig_path,buf_level=1,
                                 default_buf_size=self.graph.global_buf_size)
        self.global_buf.calculate_buf_read_power()
        ...
        for i in range(self.total_layer_num):
            ...
            if layer_type == 'conv' or layer_type == 'fc':
                self.arch_xbar_energy[i] = self.model_power.arch_xbar_power[i]*self.model_latency.total_xbar_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_ADC_energy[i] = self.model_power.arch_ADC_power[i]*self.model_latency.total_ADC_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_DAC_energy[i] = self.model_power.arch_DAC_power[i]*self.model_latency.total_DAC_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_adder_energy[i] = self.model_power.arch_adder_power[i]*self.model_latency.total_adder_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_shiftreg_energy[i] = self.model_power.arch_shiftreg_power[i]*self.model_latency.total_shiftreg_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_iReg_energy[i] = self.model_power.arch_iReg_power[i]*self.model_latency.total_iReg_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_oReg_energy[i] = self.model_power.arch_oReg_power[i]*self.model_latency.total_oReg_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_input_demux_energy[i] = self.model_power.arch_input_demux_power[i]*self.model_latency.total_input_demux_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
                self.arch_output_mux_energy[i] = self.model_power.arch_output_mux_power[i]*self.model_latency.total_output_mux_latency[i]*self.graph.layer_tileinfo[i]['avg_OU_cycle_PE']/self.graph.layer_tileinfo[i]['max_OU_cycle']/2
            else:
                self.arch_xbar_energy[i] = self.model_power.arch_xbar_power[i]*self.model_latency.total_xbar_latency[i]
                self.arch_ADC_energy[i] = self.model_power.arch_ADC_power[i]*self.model_latency.total_ADC_latency[i]
                self.arch_DAC_energy[i] = self.model_power.arch_DAC_power[i]*self.model_latency.total_DAC_latency[i]
...
if __name__ == '__main__':
    ...
    __energy = Model_energy_ou(NetStruct=structure_file,SimConfig_path=test_SimConfig_path)
